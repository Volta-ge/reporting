...
wbp = os.path.join(BASE, 'wb_data.json')
wb = open(wbp, encoding='utf-8').read() if os.path.exists(wbp) else 'null'
out = tpl.replace('__DATA__', data.replace('</script', '<\\/script')).replace('__WBDATA__', wb.replace('</script', '<\\/script'))
