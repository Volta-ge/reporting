...
<div class="page" id="page-rejectionreasons">
  ...
  <div class="page acct-scope" id="page-acct-recon">
    ...
    <div class="grid-main">
      ...
      <div class="side-stack">
        <div class="panel"><div class="panel-head"><span class="panel-title" id="reconSideTitle">მენეჯერების მიხედვით</span><span class="panel-note" id="reconSideNote">ტოპ, აკლია-ზედნადების რაოდენობით</span></div><div class="barlist" id="reconMgrBars"></div></div>
      </div>
    </div>
...
(function(){
  ...
  function renderKPIs(){
    ...
    const riskAmount = people.reduce((s,p)=> p.status !== "matched" ? s+Math.abs(p.netDiff) : s, 0);
    ...
    document.getElementById("reconKpiMissingSub").textContent = people.length ? ((missingWb.length/people.length)*100).toFixed(1)+"% მყიდველიდან" : " ";
    document.getElementById("reconKpiMissingSale").textContent = fmtInt(missingSale.length);
    document.getElementById("reconKpiMissingSaleSub").textContent = people.length ? ((missingSale.length/people.length)*100).toFixed(1)+"% მყიდველიდან" : " ";
  ...
  function renderBreakdowns(){
    ...
    if (status === "missing_sale"){
      // no CRM sale <-> no sales manager on the surplus waybills — break
      // those down by product instead, since "manager" doesn't apply.
      const prodCounts = {};
      people.forEach(p=>{
        p.wbs.filter(w=>!w.matched).forEach(w => prodCounts[w.prod] = (prodCounts[w.prod]||0)+1);
      });
      const top = Object.entries(prodCounts).sort((a,b)=>b[1]-a[1]).slice(0,12);
      document.getElementById("reconSideTitle").textContent = "პროდუქტების მიხედვით";
      document.getElementById("reconSideNote").textContent = "ტოპ, აკლია-გაყიდვის რაოდენობით";
      renderBarList("reconMgrBars", Object.fromEntries(top), k=>escapeHtml(k), ()=>"var(--warning)");
      return;
    }

    // for missing-waybill people, their unmatched CRM sales are the
    // representative "at risk" line items; for reconciled/all people, every
    // sale in range counts.
    const mgrCounts = {};
    people.forEach(p=>{
      const relevant = status === "missing_wb" ? p.sales.filter(s=>!s.matched) : p.sales;
      relevant.forEach(s => mgrCounts[s.mgr] = (mgrCounts[s.mgr]||0)+1);
    });
    const top = Object.entries(mgrCounts).sort((a,b)=>b[1]-a[1]).slice(0,12);
    document.getElementById("reconSideTitle").textContent = "მენეჯერების მიხედვით";
    document.getElementById("reconSideNote").textContent = "ტოპ, აკლია-ზედნადების რაოდენობით";
    renderBarList("reconMgrBars", Object.fromEntries(top), k=>escapeHtml(k), ()=>"var(--danger)");
  }
