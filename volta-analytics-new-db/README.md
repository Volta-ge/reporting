...
Ad Channels (Meta/Google Ads/GA4 external-API performance) lives in the separate `../volta-ad-channels/`
project, not here — see that folder's own README. `build_channels.js` there can inject it into this dashboard
as a sub-tab if that's ever wanted instead of a linked-out page.
