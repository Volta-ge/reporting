...
def pull_meta():
    ...
    rows = sorted(
        [(d["date_start"], d.get("spend", 0), d.get("impressions", 0), d.get("clicks", 0)) for d in data],
        key=lambda x: x[0],
    )
    write_tsv("channels_meta_daily.tsv", ["d", "spend", "impressions", "clicks"], rows)
...
def pull_gads():
    ...
    rows = [(r.segments.date, r.metrics.cost_micros / 1e6, r.metrics.impressions, r.metrics.clicks, r.metrics.conversions) for r in daily]
    write_tsv("channels_gads_daily.tsv", ["d", "cost", "impressions", "clicks", "conversions"], rows)
...
def pull_ga4():
    ...
    write_tsv("channels_ga4_daily.tsv", ["d", "sessions", "users", "conversions"], rows)
